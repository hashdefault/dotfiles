...
ROFI_WALLPAPER_THEME_FILE = os.path.expanduser("~/.config/rofi/themes/qtile-wallpaper.rasi")
GHOSTTY_THEME_FILE = os.path.expanduser("~/.config/ghostty/theme.ghostty")
WALLPAPER_FILE = os.path.expanduser("~/.config/qtile/wallpaper.txt")
WALLPAPER_DIRS = [
...
DEFAULT_WALLPAPER = os.path.expanduser("~/Pictures/wallpapers/neon-light-ring-huawei-mediapad-stock-3200x2560-716.jpg")
...
def _wallpaper_files():
    ...
    for directory in WALLPAPER_DIRS:
        ...
        for root, _dirs, names in os.walk(directory):
            for name in names:
                if not name.lower().endswith(WALLPAPER_EXTENSIONS):
                    continue
                path = os.path.join(root, name)
                if path not in seen:
                    seen.add(path)
                    files.append(path)
    return sorted(files, key=lambda p: _wallpaper_label(p).lower())
...
def set_wallpaper(path):
    ...
    if not os.path.isfile(path):
        return False
    _write_file(WALLPAPER_FILE, path)
    if qtile.core.name == "x11":
        try:
            subprocess.Popen(["feh", "--no-fehbg", "--bg-fill", path])
        except FileNotFoundError:
            return True
    return True
...
def _write_rofi_wallpaper_theme(t):
    vars = _theme_vars(t)
    content = f"""/* Auto-generated by qtile colorscheme */
...
element selected.normal {{
    background-color: {vars['bg_panel']};
    text-color: {vars['pink']};
}}
"""
    _write_file(ROFI_WALLPAPER_THEME_FILE, content)

def _rofi_row(label, icon=None):
    if icon:
...
def _write_ghostty_theme(theme_name, t):
    vars = _theme_vars(t)
    content = f"""# Auto-generated by qtile colorscheme -- Theme: {theme_name}
background = {vars['bg']}
foreground = {vars['fg']}
cursor-color = {vars['pink']}
selection-background = {vars['bg_alt']}
selection-foreground = {vars['fg']}

palette = 0={vars['bg']}
palette = 1={vars['red']}
palette = 2={vars['green']}
palette = 3={vars['yellow']}
palette = 4={vars['blue']}
palette = 5={vars['magenta']}
palette = 6={vars['cyan']}
palette = 7={vars['fg']}
palette = 8={vars['muted']}
palette = 9={vars['red_dim']}
palette = 10={vars['green']}
palette = 11={vars['yellow']}
palette = 12={vars['cyan_dim']}
palette = 13={vars['pink']}
palette = 14={vars['cyan']}
palette = 15={vars['fg_softest']}
"""
    _write_file(GHOSTTY_THEME_FILE, content)

def apply_external_themes(theme_name):
    theme_name = normalize_theme_name(theme_name)
    t = COLORSCHEMES[theme_name]
    _write_rofi_theme(t)
    _write_rofi_wallpaper_theme(t)
    _write_dunst_theme(t)
    _write_eww_theme(t)
    _write_ghostty_theme(theme_name, t)
    try:
    ...
    try:
        ...
        pass
    try:
        subprocess.Popen(["pkill", "-SIGUSR2", "ghostty"])
    except FileNotFoundError:
        pass
...
def choose_wallpaper(qtile):
    wallpapers = _wallpaper_files()
    if not wallpapers:
        _notify("Qtile wallpaper", "No wallpapers found in ~/Pictures/wallpapers or ~/Pictures.")
        return

    current = load_wallpaper()
    labels = []
    label_to_path = {}
    for path in wallpapers:
        label = _wallpaper_label(path)
        if path == current:
    ...
    try:
        result = subprocess.run(
            [
                ...
                "Wallpaper",
                "-theme",
                ROFI_WALLPAPER_THEME_FILE,
            ],
            input="\n".join(_rofi_row(label, label_to_path[label]) for label in labels),
            text=True,
            capture_output=True,
        )
    except FileNotFoundError:
        _notify("Qtile wallpaper", "Install rofi to use the wallpaper picker.")
        return
    ...
    selected = label_to_path.get(choice)
    if not selected:
        selected = label_to_path.get(choice.replace(" (current)", ""))
    if not selected:
        return
    if set_wallpaper(selected):
        _notify("Qtile wallpaper", _wallpaper_label(selected))

@lazy.function
def open_wallpaper_manager(qtile):
    for manager in ("azote", "nitrogen"):
...
keys = [
    ...
    Key([mod], "w", choose_wallpaper(), desc="Choose wallpaper"),
    Key([mod, "shift"], "w", open_wallpaper_manager(), desc="Open wallpaper manager"),
