...
function calendar.show(t_out, inc_offset, scr)
	...
	calendar.notification_preset.screen = scr or (calendar.followtag and awful.screen.focused())
	...
	helpers.async(f, function(ws)
		...
		calendar.notification_preset.text =
			ws:gsub("%c%[%d+[m]?%s?%d+%c%[%d+[m]?", markup.bold(markup.color(bg, fg, os.date("%e")))):gsub("\n*$", "")
...
function calendar.hover_on()
	calendar.show(0)
end
function calendar.hover_off()
	calendar.hide()
end
