...
# restart), kitty, alacritty, Fish/Tide, eww, and rofi.
#
...
FISH_THEME="$CONFIG_HOME/fish/xmonad-theme.fish"
...
# ── Fish / Tide ───────────────────────────────────────────────────────────
# Global exported colors also reach Tide's asynchronous Fish workers. Keep
# layout/icons in the user's Tide configuration; only generate palette values.
mkdir -p "${FISH_THEME%/*}"
fish_theme_tmp=$(mktemp "${FISH_THEME}.XXXXXX") || exit 1
fish_color() {
    printf 'set -gx %s %s\n' "$1" "${2#\#}"
}
{
    printf '# Auto-generated by theme-chooser - Theme: %s\n' "$chosen"
    fish_color tide_character_color "$primary"
    fish_color tide_character_color_failure "$red"
    fish_color tide_prompt_color_frame_and_connection "$muted2"
    fish_color tide_prompt_color_separator_same_color "$muted"

    for item in os context jobs private_mode status time; do
        fish_color "tide_${item}_bg_color" "$seg_purple"
        if [[ "$item" != context ]]; then
            fish_color "tide_${item}_color" "$primary"
        fi
    done
    fish_color tide_context_color_default "$text_dim"
    fish_color tide_context_color_root "$red"
    fish_color tide_context_color_ssh "$neon_cyan"
    fish_color tide_status_bg_color_failure "$red"
    fish_color tide_status_color_failure "$dark_bg"
    fish_color tide_cmd_duration_bg_color "$neon_yellow"
    fish_color tide_cmd_duration_color "$dark_bg"

    fish_color tide_pwd_bg_color "$primary"
    fish_color tide_pwd_color_anchors "$dark_bg"
    fish_color tide_pwd_color_dirs "$seg_purple"
    fish_color tide_pwd_color_truncated_dirs "$border_unfocused"
    fish_color tide_git_bg_color "$neon_green"
    fish_color tide_git_bg_color_unstable "$neon_yellow"
    fish_color tide_git_bg_color_urgent "$red"
    for state in branch conflicted dirty operation staged stash untracked upstream; do
        fish_color "tide_git_color_${state}" "$dark_bg"
    done

    for item in aws bun crystal go java kubectl pulumi python toolbox zig; do
        fish_color "tide_${item}_bg_color" "$neon_cyan"
        fish_color "tide_${item}_color" "$dark_bg"
    done
    for item in direnv distrobox docker elixir gcloud nix_shell node php ruby rustc shlvl terraform; do
        fish_color "tide_${item}_bg_color" "$neon_magenta"
        fish_color "tide_${item}_color" "$dark_bg"
    done
    fish_color tide_direnv_bg_color_denied "$red"
    fish_color tide_direnv_color_denied "$dark_bg"
    fish_color tide_vi_mode_bg_color_default "$seg_purple"
    fish_color tide_vi_mode_color_default "$primary"
    fish_color tide_vi_mode_bg_color_insert "$primary"
    fish_color tide_vi_mode_bg_color_replace "$red"
    fish_color tide_vi_mode_bg_color_visual "$neon_magenta"
    for mode in insert replace visual; do
        fish_color "tide_vi_mode_color_${mode}" "$dark_bg"
    done
} > "$fish_theme_tmp"
# Readers always see a complete palette, even during a theme switch.
mv -- "$fish_theme_tmp" "$FISH_THEME"
...
# One universal-variable event tells open Fish sessions to reload the complete
# palette and rebuild Tide's cached directory/frame colors.
if command -v fish >/dev/null 2>&1; then
    fish -c 'set -U _xmonad_fish_theme_revision $argv[1]' "$(date +%s%N)"
fi
